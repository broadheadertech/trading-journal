...
/* Module-level, so these are the same object reference on every call — a
   JS default parameter value (`dots = {...}` inline in the destructuring
   below) is otherwise a fresh object literal every single render, and both
   `dots` and `markerConfig` sit in the big scene-setup useEffect's
   dependency array further down. A caller that omits these props (or,
   worse, passes its own inline object literal that's recreated every
   render — see Hero.tsx) would otherwise cause that effect to see a
   "changed" dependency on every re-render, tearing down and rebuilding the
   entire WebGL scene each time — which is exactly what an intermittent
   flicker/disappear looks like. */
...
interface GlobeProps {
    ...
    detail?: number;
    style?: CSSProperties;
    /** Additive extension hooks — never called by anything above them in
     *  this file, so leaving both unset reproduces the exact original
     *  behavior. `onSceneReady` fires once, right after `globeGroup` exists,
     *  so a caller can parent its own Object3D children onto it (they then
     *  inherit the globe's rotation automatically, and — being real
     *  children of the same Scene — get depth-tested against the sphere
     *  mesh for free, i.e. correct far-side occlusion with no extra code).
     *  The returned cleanup (if any) runs on unmount, symmetric with this
     *  effect's own teardown below. `onFrame` fires every animation frame
     *  this component itself renders, with a live camera/globeGroup
     *  reference, so a caller can run per-frame work without needing its
     *  own separate render loop. */
    onSceneReady?: (ctx: { scene: Scene; camera: PerspectiveCamera; globeGroup: Group; globeRadius: number }) => void | (() => void);
    onFrame?: (ctx: { camera: PerspectiveCamera; globeGroup: Group }) => void;
}
...
export default function Globe({
    ...
    useEffect(() => {
        ...
        /* React StrictMode double-invokes this effect in dev (mount →
           cleanup → mount again) without unmounting the component itself,
           so `error`/`isLoading` state is shared across both invocations.
           The first invocation's async loadWorldData() fetch is still in
           flight when its cleanup disposes `renderer`/removes `canvas` —
           if that stale promise resolves afterward, `renderer.render()`
           throws on the disposed renderer, its catch block calls
           setError(...), and that error state hides the second (perfectly
           fine) globe behind the "Error loading Earth visualization"
           fallback — read as "the globe disappearing" a moment after it
           first renders. Guard every post-await continuation with this
           flag, flipped in the cleanup below, so a stale invocation's
           result is simply discarded instead of touching shared state. */
        let cancelled = false;
        ...
        const loadWorldData = async () => {
            try {
                ...
                if (cancelled) return; // stale invocation — don't clobber the live one's state
        ...
        const onSceneReadyCleanup = onSceneReady?.({ scene, camera, globeGroup, globeRadius });
